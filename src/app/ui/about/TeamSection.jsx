...
export default function TeamSection() {
  return (
    <section className="flex flex-col w-full overflow-hidden">
      ...
      {/* ================================================================== */}
      <div className="bg-background text-foreground py-20 md:py-32 px-6 md:px-12 lg:px-20 relative border-b border-border">
        {/* 
        ...
        */}
        <div className="absolute top-10 left-0 w-full overflow-hidden flex justify-center pointer-events-none opacity-[0.03] select-none">
          <h2 className="text-[10rem] md:text-[15rem] font-black whitespace-nowrap tracking-tighter text-foreground">
            NEVILLE
          </h2>
        </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <header className="text-center mb-16 md:mb-24">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-foreground mb-4 tracking-tight drop-shadow-md">
              The Hearts Behind NEVILLE
            </h2>
            <h3 className="text-2xl md:text-3xl font-medium text-primary mb-6 font-serif italic">
              Meet Our Co-Founders
            </h3>
            <p className="text-sm md:text-base text-muted-foreground max-w-lg mx-auto font-medium leading-relaxed">
          ...
          */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto">
            ...
            <div className="hidden lg:flex lg:col-start-2 lg:row-start-2 items-center justify-center flex-col opacity-20 select-none">
              ...
              <span className="font-black tracking-widest uppercase text-xl text-center leading-tight text-foreground">
                Neville
                <br />
                Youth
                <br />
            ...
            {CO_FOUNDERS.map((founder) => (
              ...
              */
              <article
                key={founder.id}
                className={`bg-card text-card-foreground border border-border rounded-[2.5rem] p-8 shadow-2xl flex flex-col items-center text-center hover:-translate-y-2 transition-transform duration-500 group ${founder.gridClass}`}
              >
                {/* 
                ...
                */}
                <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden mb-6 ring-4 ring-primary/10 group-hover:ring-primary/30 transition-all duration-300 shadow-lg bg-muted">
                  <Image
                    src={founder.image}
                ...
                <h4 className="text-xl md:text-2xl font-bold tracking-tight mb-1 underline decoration-primary/30 underline-offset-4 group-hover:decoration-primary/80 transition-colors line-clamp-1 text-foreground">
                  {founder.name}
                </h4>

                <div className="flex flex-col text-xs font-bold text-primary uppercase tracking-wider mb-4 gap-0.5">
                  <span>{founder.role}</span>
                  <span className="opacity-80 text-muted-foreground">{founder.title}</span>
                </div>

                <p className="text-sm text-muted-foreground leading-relaxed font-medium italic grow">
                  &quot;{founder.quote}&quot;
                </p>

                <MemberSocials
                  socials={founder.socials}
                  iconClass="text-muted-foreground hover:bg-primary/10"
                  hoverClass="hover:text-primary hover:scale-110"
                  containerClass="border-t border-border w-full pt-5"
                />
              </article>
            ))}
          </div>
      ...
      {/* ================================================================== */}
      <div className="bg-background text-foreground py-20 md:py-32 px-6 md:px-12 lg:px-20 relative overflow-hidden">
          ...
          {DEPARTMENTS.map((dept, index) => (
            <div key={index} className="flex flex-col gap-8 md:gap-12">
              <header className="flex flex-col">
                <h3 className="text-sm md:text-base font-medium text-primary uppercase tracking-widest mb-1">
                  Department Of
                </h3>
              ...
              */}
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
                {dept.members.map((member, idx) => (
                  <article
                    key={idx}
                    className="bg-card text-card-foreground border border-border rounded-[2.5rem] p-6 sm:p-8 text-center shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group flex flex-col items-center"
                  >
                    <div className="relative w-32 h-32 md:w-40 md:h-40 mx-auto mb-6 rounded-full overflow-hidden shadow-inner ring-4 ring-primary/20 group-hover:ring-primary/50 transition-all bg-muted">
                      <Image
                        src={member.image}
                    ...
                    <h4 className="text-lg md:text-xl font-bold text-foreground mb-1.5 line-clamp-2 leading-tight">
                      {member.name}
                    </h4>
                    ...
                    */}
                    <div className="mt-auto w-full">
                      <MemberSocials
                        ...
                        iconClass="text-muted-foreground hover:bg-primary/10"
                        hoverClass="hover:text-primary hover:scale-110"
                        containerClass="border-t border-border pt-5 w-full"
                      />
                    </div>
                  </article>
                ))}
